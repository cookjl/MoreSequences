...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(35.0, 100.0),
        rg.Point(30.0, 110.0),
        rg.Point(20.0, 110.0),
        rg.Point(15.0, 100.0),
        rg.Point(20.0, 95.0),
        rg.Point(20.0, 95.0)]
    circle = rg.Circle(rg.Point(25, 100), 10)
    answer = generate_points_on_circle(circle, 6)
...
def run_test_draw_points_on_circle():
    ...
    title = 'DRAW_POINTS_ON_CIRCLE, test 2: red circles.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(150, 150), 130)
    circle.fill_color = 'blue'
    draw_points_on_circle(window, circle, 100, 'red')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    title = 'PIZZA test 4:  50 slices, thin (thickness=1).'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
    circle.fill_color='yellow'
    pizza(window, circle, 50, 'black', 1)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for i in range(len(sequence)):
        line = rg.Line(circle.center,sequence[i])
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for i in range(len(sequence)):
        line = rg.Line(sequence[i-1],sequence[i])
        line.color=color
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for i in range(len(sequence)):
        ...
        list.append(count)
    for i in range(len(sequence)):
        line = rg.Line(sequence[i],sequence[list[i]])
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
    window.render(.5)
